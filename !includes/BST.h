...
template<typename T>
void tree_print(const tree_node<T>* root, int level= 0, 
                                    ostream& outs = cout);

//Prints Items in Tree preorder (Root, Left, Right)
template<typename T>
void tree_print_preorder(const tree_node<T>* root, ostream& outs = cout);

//Prints Items in Tree Inorder (Left, Root, Right)
template<typename T>
void tree_print_inorder(const tree_node<T>* root, ostream& outs = cout);

//Prints Items in Tree in postorder (Left, Right, Root)
template<typename T>
void tree_print_postorder(const tree_node<T>* root, ostream& outs = cout);
...
template<typename T>
void tree_to_inorder_arr(const tree_node<T>* root, int arr[], int &size);

//Returns an array of the tree postorder
template<typename T>
void tree_to_postorder_arr(const tree_node<T>* root, int arr[], int& size);
...
template<typename T>
void tree_print(const tree_node<T>* root, int level, ostream & outs)
{
...
void tree_print_debug(tree_node<T>* root, int level, ostream & outs)
{
	if (root != nullptr)
	{
		...
		outs << setw(width) << "[" << root->_item
			...
			//Print Height
			//Accessing directly so height won't get 
			//updated when this function gets called
			<< "] H:" << root->_height  << " BF: " << root->balance_factor() << endl;
...
bool tree_erase(tree_node<T>*& root, const T& target)
{
	...
	if (root == nullptr) {
		if (debug) cout << "Tree-Erase: Not Found: Root is Null" << endl;
		return false;
	}
	...
	//Error, should not go here
		...
		if (debug) cout << "ERROR IN TREE ERASE" << endl;
		assert(0);
		return false;
	}
...
T* Union(T* a1, int size1, T* a2, int size2, int& unionSize) {
	...
	for (int i = 0; i < travSize; i++) {
		...
		//Compare Elements from a1 and a2 array and set newElement to the 
